...
def plot_line(x, y, ax=None, xlabel='', ylabel='', title='', marker='.', ax_fs=16,
              t_fs=20, color='b', label=None):
    """Plot a single line."""
    if not ax:
        fig = plt.figure()
...
def plot_two_axes(x, y1, y2, markers=['o', '^'], labels=['1', '2'], 
                  colors=['r', 'b'], ms=8, ax=None, xlabel='', ylabels=[],
                           title='', ax_fs=16, t_fs=20, tk_fs=14):
    """Plots with two axes."""
    ...
    ax2 = ax1.twinx()
    # plot second axis
    ax2.plot(x, y2, marker=markers[1], ms=ms, label=labels[1], color=colors[1])
    # labels
    if len(ylabels) == 2:
